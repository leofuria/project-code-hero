...
class ContentRepository(
    ...
    override fun getContent(codeId: Int, queryOptions: HashMap<String, Any>): Single<BaseListModel<ContentItem>> {

        return Single.concat(
            localData.getContent(codeId, queryOptions),
            remoteData.fetchContent(codeId, queryOptions)
        )
            .filter { it.items != null }
            .firstOrError()
            .onErrorResumeNext { it.toSingleError(parseRemoteError(it), it.message) }
