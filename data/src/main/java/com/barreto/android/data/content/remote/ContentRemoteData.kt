...
class ContentRemoteData(
    ...
    override fun fetchContentList(queryOptions: HashMap<String, Any>): Single<BaseListModel<ContentItem>> {
        ...
        return apiClient.fetchContentList(queryOptions)
                .map { list -> list.toListModel() }
    }

    override fun fetchContent(endpoint: Int, queryOptions: HashMap<String, Any>): Single<BaseListModel<ContentItem>> {
        return apiClient.fetchContent(endpoint, queryOptions)
                .map { list -> list.toListModel() }
    }
}